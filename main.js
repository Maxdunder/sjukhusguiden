...
// ===== IMU + stabilisering (max stabilitet) =====
let filteredAccel = {x:0, z:0};
let velocity = {x:0, z:0};
const DEADZONE = 0.3;       // Endast tydliga rörelser registreras
const FRICTION = 0.2;       // Stark dämpning av hastighet
const ZERO_VEL = 0.01;      // Nollställ långsam rörelse
const RESET_TICKS = 20;     // Nollställ vid stillhet
let stillTicks = 0;

window.addEventListener('devicemotion', e => {
    const ax = (e.acceleration && isFinite(e.acceleration.x)) ? e.acceleration.x : (e.accelerationIncludingGravity?.x||0);
    const az = (e.acceleration && isFinite(e.acceleration.z)) ? e.acceleration.z : (e.accelerationIncludingGravity?.z||0);

    filteredAccel.x = ALPHA * ax + (1 - ALPHA) * filteredAccel.x;
    filteredAccel.z = ALPHA * az + (1 - ALPHA) * filteredAccel.z;

    if (Math.abs(filteredAccel.x) < DEADZONE) filteredAccel.x = 0;
    if (Math.abs(filteredAccel.z) < DEADZONE) filteredAccel.z = 0;

    if (filteredAccel.x === 0) velocity.x *= FRICTION;
    if (filteredAccel.z === 0) velocity.z *= FRICTION;

    velocity.x += filteredAccel.x * STEP_FACTOR;
    velocity.z += filteredAccel.z * STEP_FACTOR;

    if (Math.abs(velocity.x) < ZERO_VEL) velocity.x = 0;
    if (Math.abs(velocity.z) < ZERO_VEL) velocity.z = 0;

    if (filteredAccel.x === 0 && filteredAccel.z === 0 && velocity.x === 0 && velocity.z === 0) {
        stillTicks++;
    } else {
        stillTicks = 0;
    }
    if (stillTicks > RESET_TICKS) {
        marker.position.x = 0;
        marker.position.z = 0;
        stillTicks = 0;
    }
...
// ===== Skapa knapp för återställning av markörposition =====
...
// ===== Kamera: Inga ändringar behövs, men du kan lägga till kameraupdate/manual reset om önskas =====
