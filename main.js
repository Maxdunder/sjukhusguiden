...
let USE_WIFI = true;
let DEBUG_MODE = false; // debug-skala
let gpsWatchId = null;
let fastWifi = false;

// Marker referensposition för GPS och Wi-Fi
let markerScenePos = new THREE.Vector3(0, 1, 0);
...
marker.scale.set(2,2,2); // större så den syns bättre
marker.position.copy(markerScenePos);
scene.add(marker);

// ===== Destination & navigeringslinje =====
const destination = new THREE.Vector3(5, 1, -5);
...
function updateCamera() {
    if (FOLLOW_MARKER) {
        ...
        const desiredTarget = new THREE.Vector3(marker.position.x, 1, marker.position.z);
...
function wifiSimulate() {
    ...
    marker.position.x = wifiBasePos.x + (Math.random() - 0.5) * speed;
    marker.position.z = wifiBasePos.z + (Math.random() - 0.5) * speed;
...
function gpsUpdate(position) {
    ...
    marker.position.set(markerScenePos.x + dx, 1, markerScenePos.z + dz);
...
document.getElementById('toggle-source').addEventListener('click', () => {
    ...
    if (!USE_WIFI) {
        markerScenePos.copy(marker.position); // GPS referens
        startGPS();
    } else {
        wifiBasePos.copy(marker.position); // Wi-Fi referens
        if (gpsWatchId !== null) navigator.geolocation.clearWatch(gpsWatchId);
    }
});
