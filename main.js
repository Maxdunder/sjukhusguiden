...
const DEADZONE = 0.05;       // Mindre känsliga rörelser ignoreras
const FRICTION = 0.8;        // Större dämpning av hastighet
const ZERO_VEL = 0.001;      // Gräns för när hastighet nollställs
...
window.addEventListener('devicemotion', e => {
    ...
    // Dödzon – ignorera små rörelser
    if (Math.abs(filteredAccel.x) < DEADZONE) filteredAccel.x = 0;
    if (Math.abs(filteredAccel.z) < DEADZONE) filteredAccel.z = 0;

    // Hastighet: dämpning/friktion vid noll läge
    if (filteredAccel.x === 0) velocity.x *= FRICTION;
    if (filteredAccel.z === 0) velocity.z *= FRICTION;

    velocity.x = velocity.x + filteredAccel.x * STEP_FACTOR;
    velocity.z = velocity.z + filteredAccel.z * STEP_FACTOR;

    // Nollställ långsam drift
